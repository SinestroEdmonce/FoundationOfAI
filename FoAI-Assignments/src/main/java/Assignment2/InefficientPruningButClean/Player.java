...
public class Player {
    ...
    public Queue<Action> generateInsideCampActions(Board board) {
        // Action queue to return
        Queue<Action> actions = new LinkedList<>();

        // Heap to obtain move-furthest action
        Queue<Action> furthestActions = new PriorityQueue<>(new Comparator<Action>() {
            int []center = ownCamp.getCenter();

            @Override
            public int compare(Action o1, Action o2) {
                int []o1Coord = o1.getNewCoord();
                int []o2Coord = o2.getNewCoord();
                int o1Dist = Math.abs(o1Coord[0]-this.center[0])+Math.abs(o1Coord[1]-this.center[1]);
                int o2Dist = Math.abs(o2Coord[0]-this.center[0])+Math.abs(o2Coord[1]-this.center[1]);

                return o2Dist-o1Dist;
            }
        });

        List<Integer> piecesInCamp = new ArrayList<>();
        for (int idx=0; idx<this.piecesList.size(); ++idx) {
            // If in camp
            if (this.piecesList.get(idx).getStatusOfPieces() == PiecesStatus.IN_OWN_CAMP) {
                piecesInCamp.add(idx);
            }
        }

        this.generateAllActions(furthestActions, board, piecesInCamp);

        return furthestActions;
    }

    public Queue<Action> generateOutsideCampActions(Board board) {
        // Action queue to return
        Queue<Action> actions = new LinkedList<>();
        List<Integer> piecesOutCamp = this.piecesIdx;

        this.generateAllActions(actions, board, piecesOutCamp);

        return actions;
    }

    private void generateAllActions(Queue<Action> actions, Board board, List<Integer> piecesIndices) {
        for (int itr=0; itr<piecesIndices.size(); ++itr) {
            Pieces currPieces = this.piecesList.get(piecesIndices.get(itr));
            ...
            for (int []di: this.diAct) {
                ...
                if (this.isValid(newCoord[0], newCoord[1], board)
                    ...
                    actions.add(new ActionMove(oldCoord, newCoord,
                            currPieces.getTypeOfPieces(), PlayerType.EMPTY, piecesIndices.get(itr)));
                    isVisited[newCoord[0]][newCoord[1]] = true;
                }
            }

            // Generate Jump
            this.generateJump(actions, board, piecesIndices.get(itr), isVisited);
        }
    }
    ...
    public int evaluate() {
        ...
        return (10*totalPieces-7*totalDist);
    }
    ...
    public double countDelta(Action action, int deltaMeInOppositeCamp, double depth) {
        ...
        return (double)(10*deltaMeInOppositeCamp-7*deltaDist)*(1.0-depth/10000.0);
    }
}
